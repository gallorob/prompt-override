...
class ChatWidget(Widget):
	def __init__(self, game_screen: Screen, level: Level, karma: Karma):
		...
		self.title = Static(self._title.replace('$UNREAD$', self._unread_str()), classes="horizontal-centered", id="chat_title")
		self.history = ScrollableContainer(Static("", markup=True, expand=True, id="chat_history"))
		self.input = Input(placeholder="Type a message...", id="chat_input")

		self.watch(self.title, attribute_name='has_focus', callback=self.update_title, init=False)
		self.watch(self.history, attribute_name='has_focus', callback=self.update_title, init=False)
		self.watch(self.input, attribute_name='has_focus', callback=self.update_title, init=False)
